                    ...
                    <table class="table table-hover text-nowrap w-50">
                        ...
                        @foreach($users as $user)
                            <tr>
                                ...
                                <td>{{$user->email}}</td>
                                @if(\App\Models\UserTime::where('user_id', $user->id)->count() >= 1)
                                <td class="text-center"><a href="{{route('user.report', $user->id)}}">Составить отчёт</a></td>
                                @else
                                    <td class="text-center"><a href="{{route('user.change.time', $user->id)}}" class="text-success">Добавьте время посещения</a></td>
                                @endif
                                <td class="text-center"><a href="{{route('user.change.time', $user->id)}}" class="text-success">Время посещения</a></td>
